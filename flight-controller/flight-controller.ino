...
float pid_p_gain_roll = 1.3;               //Gain roll P-controller (1.3)
float pid_i_gain_roll = 0.04;              //Gain roll I-controller (0.05)
float pid_d_gain_roll = 18.0;                //Gain roll D-controller (15)
...
float pid_p_gain_yaw = 4;                  //Gain yaw P-controller. //4.0
...
float bttry_compens;
float angle_pitch, angle_roll, angle_roll_acc, angle_pitch_acc;
...
void setup(){
  ...
  loop_timer = micros(); //Set the loop timer for next loop
...
void loop() {
  ...
  if(micros() - loop_timer > 4050) PORTB |= _BV(PB4);  //Turn on the LED if the loop time exceeds 4050us.
